import{g as e,h as t,l as n,t as r}from"./index-DcmvT0CB.js";var i=()=>`
    ...
    `,a=async()=>{let i=await t(),a=document.querySelector(`.brand-name-ticket`);a&&(a.textContent=(i.brandName||`NEXFIT`)+` GYM`);let o=document.getElementById(`btnActivarCamara`),s=document.getElementById(`ticketContainer`),c=null,l=null,u=async t=>{let i=(await e()).find(e=>e.id===t);if(!i){window.showToast(`Código no reconocido`,`danger`);return}c=i,s.style.opacity=`1`,s.style.pointerEvents=`auto`,document.getElementById(`tktSocio`).textContent=i.nombre,document.getElementById(`tktPlan`).textContent=`Plan ${i.membresia}`,document.getElementById(`tktVenc`).textContent=n(i.fechaVencimiento);let a={day:`2-digit`,month:`2-digit`,year:`numeric`,hour:`2-digit`,minute:`2-digit`};document.getElementById(`tktFecha`).textContent=new Date().toLocaleDateString(`es-ES`,a);let o=document.getElementById(`tktStatus`);i.estado===`Activo`?(o.textContent=`✅ ¡ACCESO PERMITIDO!`,o.className=`ticket-status success`,await r(i.id,i.nombre),window.showToast(`Acceso concedido a ${i.nombre}`,`success`)):(o.textContent=`❌ MEMBRESÍA VENCIDA`,o.className=`ticket-status denied`,window.showToast(`Atención: Membresía de ${i.nombre} vencida`,`danger`)),l&&(l.pause(!0),setTimeout(()=>l.resume(),3e3))};o&&o.addEventListener(`click`,()=>{l||(document.getElementById(`qr-reader`).innerHTML=``,o.style.display=`none`,l=new window.Html5QrcodeScanner(`qr-reader`,{fps:10,qrbox:250},!1),l.render(e=>u(e),e=>{}))}),document.getElementById(`btnImprimir`).addEventListener(`click`,()=>{let e=document.querySelector(`.ticket`);if(!e)return;let t=window.open(``,`_blank`,`width=400,height=600`);t.document.write(`
